...
* 备  注 : 无
********************************************************************************/
void  SortAver_FilterXYZ(IMU_Struct *IMU_Structure,float_Struct *Acc_filt,uint8_t n)
{
	static float bufx[N],bufy[N],bufz[N];
	static uint8_t cnt =0,flag = 1;
	float temp1=0,temp2=0,temp3=0;
	uint8_t i;
	bufx[cnt] = IMU_Structure->AccX;
	bufy[cnt] = IMU_Structure->AccY;
	bufz[cnt] = IMU_Structure->AccZ;
...
* 备  注 : 无
*********************************************************************************/
void Aver_FilterXYZ6(IMU_Struct *IMU_Structure,float_Struct *Acc_filt,float_Struct *Gry_filt,uint8_t n)
{
	static float bufax[N],bufay[N],bufaz[N],bufgx[N],bufgy[N],bufgz[N];
	static uint8_t cnt =0,flag = 1;
	float temp1=0,temp2=0,temp3=0,temp4=0,temp5=0,temp6=0;
	uint8_t i;
	bufax[cnt] = IMU_Structure->AccX;
	bufay[cnt] = IMU_Structure->AccY;
	bufaz[cnt] = IMU_Structure->AccZ;
	bufgx[cnt] = IMU_Structure->GyroX;
	bufgy[cnt] = IMU_Structure->GyroY;
	bufgz[cnt] = IMU_Structure->GyroZ;
...
* 功  能 ：滑动窗口滤波三组数据(無去最值)
* 参  数 ：*IMU_Structure 要滤波数据地址
*          *Acc_filt 滤波后数据地址
* 返回值 ：无
* 备  注 : 无
*******************************************************************************/
void Aver_FilterXYZ(IMU_Struct *IMU_Structure,float_Struct *Acc_filt,uint8_t n)
{
